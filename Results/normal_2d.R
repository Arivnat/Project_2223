...
normal3<-function(n){
  ...
  for (i in 1:1000){
    ...
    y3<-2+3.5*x1+2.5*x2+rgamma(n,1,2)*exp(0.05*(x1+x2))
    y4<-2+3.5*x1+2.5*x2+rgamma(n,2,2)*exp(0.05*(x1+x2))
    y5<-2+3.5*x1+2.5*x2+rgamma(n,5,1)*exp(0.05*(x1+x2))
    y6<-2+3.5*x1+2.5*x2+rgamma(n,7,3)*exp(0.05*(x1+x2))
    ##student's t
    y7<-2+3.5*x1+2.5*x2+rt(n,2)*exp(0.05*(x1+x2))
    y8<-2+3.5*x1+2.5*x2+rt(n,3)*exp(0.05*(x1+x2))
    ##lognormal
    y9<-2+3.5*x1+2.5*x2+rlnorm(n,0,1)*exp(0.05*(x1+x2))
    y10<-2+3.5*x1+2.5*x2+rlnorm(n,1,3)*exp(0.05*(x1+x2))
    y11<-2+3.5*x1+2.5*x2+rlnorm(n,0,3)*exp(0.05*(x1+x2))
    y12<-2+3.5*x1+2.5*x2+rlnorm(n,1,2)*exp(0.05*(x1+x2))
    ##Pareto
    y13<-2+3.5*x1+2.5*x2+rPareto(n,1,2)*exp(0.05*(x1+x2))
    y14<-2+3.5*x1+2.5*x2+rPareto(n,1,3)*exp(0.05*(x1+x2))
    y15<-2+3.5*x1+2.5*x2+rPareto(n,2,3)*exp(0.05*(x1+x2))
...
start<-Sys.time()
na<-normal3(50)  ###6.26 mins
Sys.time()

start<-Sys.time()
nb<-normal3(70)   ###11.26727 mins
Sys.time()-start

start<-Sys.time()
n1<-normal3(100) ##Warning msg-glm ###13.92789 mins 
Sys.time()-start
##glm.fit did not converge error-https://www.statology.org/glm-fit-algorithm-did-not-converge/

##to do:
start<-Sys.time()
n2<-normal3(200) ###23.77494 mins
Sys.time()-start

start<-Sys.time()
n3<-normal3(300)   ###4.32699 mins
Sys.time()-start 

start<-Sys.time()
n4<-normal3(500)  ###1.332094 hours
Sys.time()-start


library(openxlsx)
write.xlsx(na,"normal_x1x2na.xlsx")
write.xlsx(nb,"normal_x1x2nb.xlsx")
write.xlsx(n1,"normal_x1x2n1.xlsx")
write.xlsx(n2,"normal_x1x2n2.xlsx")
write.xlsx(n3,"normal_x1x2n3.xlsx")
write.xlsx(n4,"normal_x1x2n4.xlsx")

###########################################################################################
#############################################################################################
###########################Finding avg coverage, avg.length,sd###############################
#############################################################################################
...
n0a<-lapply(1:13, function(i) read_excel("normal_x1x2na.xlsx", sheet = i))
n0b<-lapply(1:13, function(i) read_excel("normal_x1x2nb.xlsx", sheet = i))
n0c<-lapply(1:13, function(i) read_excel("normal_x1x2n1.xlsx", sheet = i))
n0d<-lapply(1:13, function(i) read_excel("normal_x1x2n2.xlsx", sheet = i))
n0e<-lapply(1:13, function(i) read_excel("normal_x1x2n3.xlsx", sheet = i))
n0f<-lapply(1:13, function(i) read_excel("normal_x1x2n4.xlsx", sheet = i))

###coverage

c0a<-lapply(14:26, function(i) read_excel("normal_x1x2na.xlsx", sheet = i))
c0b<-lapply(14:26, function(i) read_excel("normal_x1x2nb.xlsx", sheet = i))
c0c<-lapply(14:26, function(i) read_excel("normal_x1x2n1.xlsx", sheet = i))
c0d<-lapply(14:26, function(i) read_excel("normal_x1x2n2.xlsx", sheet = i))
c0e<-lapply(14:26, function(i) read_excel("normal_x1x2n3.xlsx", sheet = i))
c0f<-lapply(14:26, function(i) read_excel("normal_x1x2n4.xlsx", sheet = i))

###xtest

x0a<-lapply(27:39, function(i) read_excel("normal_x1x2na.xlsx", sheet = i))
x0b<-lapply(27:39, function(i) read_excel("normal_x1x2nb.xlsx", sheet = i))
x0c<-lapply(27:39, function(i) read_excel("normal_x1x2n1.xlsx", sheet = i))
x0d<-lapply(27:39, function(i) read_excel("normal_x1x2n2.xlsx", sheet = i))
x0e<-lapply(27:39, function(i) read_excel("normal_x1x2n3.xlsx", sheet = i))
x0f<-lapply(27:39, function(i) read_excel("normal_x1x2n4.xlsx", sheet = i))
...
bpc(c0a,c0b,c0c,c0d,c0e,c0f,2)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,3)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,4)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,5)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,6)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,7)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,8)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,9)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,10)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,11)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,12)
bpc(c0a,c0b,c0c,c0d,c0e,c0f,13)
